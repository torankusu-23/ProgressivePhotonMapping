...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        //可以用getEmitter试一试，主要不知道enum::EEmitter的值灵不灵
        ...
        while (true)
        {
            ...
            if (x.mesh->getBSDF()->isDiffuse())
            {
                ...
                for (int i = 0; i < len; ++i)
                {
                    ...
                    local_power += wait_albedo * fr * photon.getPower();
                }
                if (local_power.maxCoeff() > 0.f)
                    result += local_power / (emittedPhotons * M_PI * m_photonRadius * m_photonRadius);
