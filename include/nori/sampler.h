...
{   
    ...
    ViewQueryRecord(Intersection its, Point2f bitmap_pos, 
    Vector3f dir, Color3f power, float radius, Color3f albedo, uint32_t p_nums):
    its(its), bitmap_pos(bitmap_pos), dir(dir), power(power), radius(radius), albedo(albedo), p_nums(p_nums)
    { }
};
